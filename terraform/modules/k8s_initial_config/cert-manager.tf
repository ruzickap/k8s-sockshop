...
resource "null_resource" "cert-manager-crds" {
  ...
  triggers = {
    template_file_cert-manager_application_sha1 = "${sha1("${data.http.cert-manager_crds.body}")}"
  }

  provisioner "local-exec" {
    command = "kubectl apply --kubeconfig=${var.kubeconfig} -f ${data.http.cert-manager_crds.url}"
  }
...
resource "helm_release" "cert-manager" {
  depends_on = [null_resource.cert-manager-crds, kubernetes_namespace.cert-manager, kubernetes_cluster_role_binding.tiller]
  name       = "cert-manager"
